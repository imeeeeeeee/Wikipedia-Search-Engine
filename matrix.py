...
def update_word_page_relation(page_id, text, word_page_relation, valid_words):
    ...
    for word in page_words:
        if word in valid_words:
            page_word_count[word] += 1
    for word, count in page_word_count.items():
        word_page_relation[word][page_id] += count

    # ### doc nlp was way too long. so we decided to remove it for now
    # doc = nlp(text)
    # for token in doc:
...
def process_xml_file(xml_data, valid_words=valid_words, prints_every=100):
    # root = ET.fromstring(xml_data)  # Parse the XML data
    ns = {'mw': 'http://www.mediawiki.org/xml/export-0.10/'}

    tree = ET.parse(xml_data) 
    root = tree.getroot()
    # Initialize CLI representation components for the adjacency matrix
    C, I, L = [], [], [0]
    ...
    for page in root.findall('.//mw:page',ns):
        DEBUG_COUNTER+=1 #putting before so that it doesn't say "0 pages read"
        if DEBUG_COUNTER%prints_every==0 :
            print(this_file,'read',DEBUG_COUNTER, 'pages...')
        page_title = page.find('.//mw:title',ns).text
        page_text = page.find('.//mw:text',ns).text
        page_id = pagetitle_to_pageid[page_title]
        
        # Update word-page relation based on current page
        update_word_page_relation(page_id, page_text, word_page_relation, valid_words)

        # Find and process internal links in the current page
        links = find_internal_links(page_text)
        for link in links:
            # link_id = RELATIONS[link.lower()]
            link_id = pagetitle_to_pageid.get(link)
    ...
    print(this_file,': all',DEBUG_COUNTER,'pages read.')
    
    print(this_file,': creating CLI.')
    # Build the CLI representation as we go
    for page_id in range(0, max(adjacency_list.keys()) + 1): #not sure about that +1 (TODO)
    ...
    # Also return CLI representation for further use
    return C, I, L
...
# Parse and process the XML data
    ...
    print(this_file,': computing TF and ND.')
    tf = compute_tf()
    nd = compute_nd()
    ...
    # word_pages_list = {word: list(pages.keys()) for word, pages in word_page_relation.items()} 

    # print(word_pages_list)
    # with open('WPR.pkl','wb') as file:
    #     pickle.dump(word_pages_list,file)
    
    with open('WPR.pkl', 'wb') as f:
        dill.dump(word_page_relation, f)
